...
class ConfigurationError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
    """Raised when a required configuration value is missing or invalid."""


class ProviderError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
...
class GenerationError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
    """Raised when LLM generation fails (API error, timeout, invalid response)."""


class RetrievalError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
    """Raised when the retrieval pipeline fails (vector store error, no results)."""


class IngestionError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
    """Raised when document ingestion fails (parse error, embedding error, etc.)."""


class MemoryError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):  # noqa: A001
    """Raised when reading from or writing to the memory store fails."""


class PromptNotFoundError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
    """Raised when a prompt template file cannot be found on disk."""


class RateLimitError({{ cookiecutter.project_slug | title | replace('_', '') }}Error):
...
# Example: class DocumentNotFoundError({{ cookiecutter.project_slug | title | replace('_', '') }}Error): ...
